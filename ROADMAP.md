...
- **Past-day log editing + agent backdating** (FEA-44) — Daily Log card is now editable on any historical day; writes route to `dailyLogs[date].log` instead of `todayPlan.log`. The "ask agent to summarize" button on a historical day prompts the agent to summarize THAT specific day with `set_today_log({ date: 'YYYY-MM-DD', log: ... })`. New optional `date` parameter on `set_today_log` so the agent can write retroactive summaries cleanly. Useful when you don't get to the daily log until the next morning.
- **Richer per-day digest for retroactive context** (FEA-45) — Digest's "Recent days" section bumped from 3 → 7 days and now includes per-day plan with status pips (`STP-4✓ MZ-6· PER-1○`), the focus note, and a list of subtasks completed on that day (attributed by `completedAt`). Lets the agent write meaningful retroactive summaries grounded in actual completions, not just whatever Mark says.
- **Agent task name resolution** (FEA-46) — Agent can now resolve any task (active OR DONE) by name without Mark needing to remember IDs. Two digest changes: (1) per-workstream "done" roll-up line lists every DONE task as `id (title)`; (2) flat alphabetical Task Index at the end of the digest lists every task with `[done]` / `[waiting]` tags for instant lookup. New top-level rule #6 in the system prompt directs the agent to scan the index and act on the resolved id directly — only asking when the lookup is ambiguous (multiple matches → list candidates) or empty. No more "I don't see a Travel Task in your active workstreams, what's the context?" — agent finds PER-1 and adds the subtask immediately.

### v0.8.1 — Apr 27, 2026 — Priority-queue subtask context + sharper intake
- **Active subtask hint on priority queue rows** (FEA-40) — Each task in the priority queue now shows the most-urgent open subtask (`↳ Fix the revenue decomposition panel · May 1`) under the task title when collapsed. Picker prefers overdue → due-within-2-days → first not-done. Due date renders red when overdue. Makes "what specifically am I working on" obvious without expanding the row.
- **Other Active rows are now expandable** (FEA-41) — Each other-active row gets a chevron to drop down its outstanding subtasks (with due-date pills, overdue / due-soon coloring). Collapsed view shows the next-up subtask plus a `+N` count for additional open items. "Add to today" is now an explicit `+ today` button so clicking the row body opens the dropdown instead of accidentally adding the task.
- **Sharper, secretary-style morning intake** (FEA-42) — Rewrote `MORNING_INTAKE_BLOCK`. Reframes the agent as Mark's "morning chief-of-staff": drills for specifics ("which subtask of STP-4 specifically?"), surfaces forcing functions (calendar pressure, deadlines, rollover causes, energy/sequencing), pushes back on overlong priority lists, and ties focus_note to the actual work being done — not generic phrasing.
...
|----|------|------|----------|---------|
| FEA-12 | **Calendar Awareness (private ICS feed)** | Feature | **High** | Make the agent calendar-aware via Google Calendar's "Secret address in iCal format" — no GCP project, no OAuth, no admin access required. **Setup (one-time, ~2 min on user side):** Google Calendar → Settings → Integrate calendar → copy "Secret address in iCal format" → paste into Workplan Context tab. **Build (~2-3 hrs):** (1) ICS-URL field in Context tab, encrypted-at-rest in Supabase, masked after entry. (2) Netlify function `/api/calendar-events?date=…` that fetches the URL server-side, parses with `ical.js` (handles recurring events, time zones), and returns today/tomorrow's events as JSON. Server-side fetch keeps URL out of browser logs. (3) Today-view meetings strip — compact "9:00 standup · 11:30 1:1 with May · 1:30 CTO rehearsal" — with caching (TTL ~30 min). (4) Agent digest gets a "TODAY'S CALENDAR" block listing remaining meetings + free blocks. Morning intake can now ask sharper questions: "1:30 CTO rehearsal — how much prep?" Stalled-task nudges become meeting-aware. **Tradeoffs vs OAuth:** read-only forever (no write-back), Google-controlled refresh cadence (~hours, not real-time). Both are fine — read-only is the only thing we'd build anyway, and morning planning doesn't need real-time. **Out of scope:** OAuth flow, write-back to calendar, multi-account. Defer indefinitely unless a clear need surfaces. | calendar-aware — agent picks tasks that fit between meetings rather than 8 deep-work items on a meeting-heavy day. (5) Optional: detect meeting cancellations and prompt "you got an hour back — bump SEG-4 up?". |
...
| FEA-40 | **Active subtask hint on priority queue** — Each task row in the Today priority queue shows the most-urgent open subtask (`↳ subtitle · due-date`) under the title. Picker prefers overdue → due-within-2-days → first not-done. | Feature | Apr 27 |
| FEA-41 | **Other Active rows expandable** — Other-active rows on Today view get a chevron that drops down all outstanding subtasks with due-date pills. Adds explicit `+ today` button so row body click expands instead of accidentally adding the task. | Feature | Apr 27 |
| FEA-42 | **Sharper morning intake prompt** — Rewrote `MORNING_INTAKE_BLOCK` as "morning chief-of-staff": drills for specifics, surfaces forcing functions (calendar/deadlines/rollovers), pushes back on overlong lists, ties focus_note to actual work. | Feature | Apr 27 |
| FEA-43 | **Backdated subtask completions in history view** — Past-day view in TodayView is no longer fully read-only. Subtask toggles work and `completedAt` is backdated to noon-local on the viewed date. `toggleSubtask` mutation accepts `{ effectiveDate }`. Plan-level controls remain locked. History banner reworded to explain the semantics. | Fix | Apr 30 |
