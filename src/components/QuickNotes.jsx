...
export default function QuickNotes({ notes, readOnly, onAdd, onDelete }) {
    ...
    <>
      ...
      {notes.length === 0 && (
        <div style={{ padding: "12px 0", fontSize: "12px", color: "#3A3A3E", fontFamily: "'DM Sans', sans-serif", fontStyle: "italic" }}>
          {readOnly ? "No notes in this week" : "No notes yet — jot something down"}
        </div>
      )}
      {notes.map((n, i) => (
        <div key={i} style={{ display: "flex", gap: "8px", padding: "6px 0", borderBottom: i < notes.length - 1 ? "1px solid #1C1C1E" : "none" }}>
          ...
          {!readOnly && <button onClick={() => onDelete(i)} style={{ background: "transparent", border: "none", color: "#3A3A3E", cursor: "pointer", fontSize: "12px", padding: mobile ? "4px 8px" : "0 4px", minWidth: mobile ? "44px" : "auto", minHeight: mobile ? "44px" : "auto", display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>}
        </div>
      ))}
    </>
  );
