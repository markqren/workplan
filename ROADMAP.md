...
**Last updated:** Mar 3, 2026 | Stack: Vite + React + Supabase + Netlify Functions
...
| FEA-19 | **Agent Undo (Snapshot Cache)** | Feature | **High** | Add ability to undo agent actions when the agent messes up. (1) Before `handleAgentActions` applies any actions, snapshot the current tracker data (deep clone). Store in a small in-memory ring buffer (last 5 snapshots) with the agent message that triggered them. (2) After agent actions are applied, show an "Undo" button in the agent panel next to the agent's response (or as a toast). Clicking it restores the pre-action snapshot via `persist()`. (3) Undo button disappears after subsequent manual edits or after a timeout (e.g. 30s). (4) Optionally show a summary of what the agent changed ("added 2 tasks, updated 1 status") so the user can decide whether to undo. |
